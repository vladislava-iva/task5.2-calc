...
void gauss3(double A[3][4],double& ra,double& rb, double& rc)
{
    ...
    cout<< "матрица после приведения к диаг. виду:\n";
    for(int r=0;r<3;r++){
        for(int j=0;j<=3;j++) cout<< A[r][j]<<" ";
        cout<< endl;
    }
...
int main()
{
    ...
    double a2, b2;
    kram2(sXX, sX, sX, n, sXY, sY, a2, b2);//теперь обычное мнк
    double be2 = exp(b2);//b=e^be
